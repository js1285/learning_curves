...
# def extrapol_plot(cnn_after, x, ys, xlabel, ylabel, title=""):
def results_plot(params, results, title="L1 regularisation"):
    
   
    plt.figure()
    plt.semilogx()
    # for (ylabel, y) in ys.items():
    line_pred = plt.plot(params, results)
    plt.title(title)
    # plt.xlabel(xlabel)    
    # plt.legend([line_1,line_2], ['frozen cnn layers', 'unfreezing top 2 cnn layer-blocks'])
    plt.legend()
    ...
    path = os.path.join("plots/", title+'_'+'.png')
    plt.savefig(path)    
...
def load_data(source_dir='./data/final_project', scale_configs = True, data_from = 'local'):
    ...
    if data_from == 'local':
        ...
        file_service = FileService(account_name='jochenstorage', account_key='7onD5l6X5dfmfUC+gXJCnKRn5AwPRXz8lEDaIsTVH5Di5y0wFTWgitc6Rq2TV85Zjx+EOMlRJYsxFHt1R41qPA==')            
        generator = file_service.list_directories_and_files('jochenfileshare/dlproject_data/final_project')
